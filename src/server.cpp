...
void* RevMsg( void *cfd )
{
	...
	while( recv(connfd, recv_buf, sizeof(recv_buf), 0) > 0 ) // 接收数据
	{
		...
		if( state == FREE && strcmp(recv_buf, "GetListClient") == 0 ) {
			for( vector<char>::iterator i = ListClient.begin(); i != ListClient.end(); i++ ) {
				MsgBack += (*i);
				if( *i == ( connfd + '0' ) )
					MsgBack += "(me)" ;
		...
		if ( send(connfd, MsgBack.c_str(), MsgBack.size(), 0) < 0)   // 向服务器发送信息
